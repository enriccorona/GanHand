...
## LOAD GT OBJECTS AND GRASPS FOR SCENE:
        ...
        available_repr = np.load('/tmp-network/fast/ecorona/data/YCB_objects/allpossiblegrasps/mano_representation_%d.npy'%(self.indexs_split[index]), allow_pickle=True)
        available_trans = np.load('/tmp-network/fast/ecorona/data/YCB_objects/allpossiblegrasps/mano_translation_%d.npy'%(self.indexs_split[index]), allow_pickle=True)
        available_taxonomies = np.load('/tmp-network/fast/ecorona/data/YCB_objects/allpossiblegrasps/mano_taxonomy_%d.npy'%(self.indexs_split[index]), allow_pickle=True)
...
## LOAD POSE PREDICTED BY POSE ESTIMATION METHOD
        predPose = np.load('/tmp-network/fast/ecorona/predictions/YCB-Video-Out_per_image/pred_'+str(self.indexs_split[index])+'.npy', allow_pickle=True)
        all_obj_verts = []
        all_obj_faces = []
        obj_bboxes = []
        obj_poses = []
    ...
    def _read_dataset_paths(self):
        ...
        models = glob.glob('/tmp-network/fast/ecorona/data/YCB_objects/0*/google_16k/textured.obj')
        models.sort()
