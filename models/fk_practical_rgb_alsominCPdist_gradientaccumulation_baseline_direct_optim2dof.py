    ...
    def forward(self, keep_data_for_visuals=False):
        if not self._is_train:
            ...
            for i in range(self._B):
                handfullpose_list, touching_indexs, loss_distance, loss_angle, loss_angle_2ndjoint = fk.optimize_hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._input_obj_verts[i][::5], step=5) # Just doing every 5 points to not collapse gpu mem
