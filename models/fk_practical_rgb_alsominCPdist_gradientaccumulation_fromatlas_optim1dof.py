...
class Model(BaseModel):
    ...
    def set_input(self, input):
        self._input_rgb_img = input['rgb_img'].float().permute(0, 3, 1, 2)
    ...
    def forward(self, keep_data_for_visuals=False):
        if not self._is_train:
            ...
            all_touching_indexs = []
            for i in range(self._B):
                handfullpose_list, touching_indexs, loss_distance, loss_angle = fk.optimize_hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._predicted_obj_verts[i].cpu().data.numpy(), step=5) # Just doing every 5 points to not collapse gpu mem
                print(touching_indexs)
    ...
    def _forward_G(self, keep_data_for_visuals):
        ...
        if self._opt.lambda_G_angles > 0 or self._opt.lambda_G_fk > 0:
            ...
            loss_angles = []
            for i in range(self._B):
                handfullpose_list, touching_indexs, loss_distance, loss_angle = fk.optimize_hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._predicted_obj_verts[i].cpu().data.numpy()) # Just doing every 5 points to not collapse gpu mem
                loss_fk.append(loss_distance)
                loss_angles.append(loss_angle)
            self._loss_g_fk = torch.stack(loss_fk).mean()*self._opt.lambda_G_fk
            self._loss_g_angles = torch.stack(loss_angles).mean()*self._opt.lambda_G_angles
