...
# IF IT WAS ORIGINAL FK, IT WOULD BE:
# handfullpose = fk.get_fullhandpose(HR) 
#             loss_fk = [] 
#             loss_angles = [] 
#             handfullposes = [] 
#             for i in range(self._B): 
#                 handfullpose_list, touching_indexs, loss_distance, loss_angle = fk.optimize_
# hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._input_obj_ver
# ts[i], self._input_obj_faces[i]) # Just doing every 5 points to not collapse gpu mem 
...
class Model(BaseModel):
    ...
    def forward(self, keep_data_for_visuals=False):
        if not self._is_train:
            ...
            all_touching_indexs = []
            for i in range(self._B):
                handfullpose_list, touching_indexs, loss_distance, loss_angle = fk.optimize_hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._input_obj_verts[i][::5], step=5) # Just doing every 5 points to not collapse gpu mem
                print(touching_indexs)
        ...
        if self._opt.lambda_G_angles > 0 or self._opt.lambda_G_fk > 0:
            ...
            for i in range(self._B):
                if self._opt.is_train:
                    handfullpose_list, touching_indexs, loss_distance, loss_angle = fk.optimize_hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._input_obj_verts[i][::5]) # Just doing every 5 points to not collapse gpu mem
                else:
                    handfullpose_list, touching_indexs, loss_distance, loss_angle = fk.optimize_hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._input_obj_verts[i], step=5) # Just doing every 5 points to not collapse gpu mem
                loss_fk.append(loss_distance)
                loss_angles.append(loss_angle)
            self._loss_g_fk = torch.stack(loss_fk).mean()*self._opt.lambda_G_fk
            self._loss_g_angles = torch.stack(loss_angles).mean()*self._opt.lambda_G_angles
