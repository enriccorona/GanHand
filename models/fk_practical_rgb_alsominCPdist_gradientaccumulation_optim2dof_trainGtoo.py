...
from utils import forward_kinematics_2dof as fk
...
class Model(BaseModel):
    ...
    def forward(self, keep_data_for_visuals=False):
        if not self._is_train:
            ...
            self._loss_g_CE = self._criterion_CE(prediction, self._input_label)
            ...
            all_touching_indexs = []
            for i in range(self._B):
                handfullpose_list, touching_indexs, loss_distance, loss_angle, loss_angle_2ndjoint = fk.optimize_hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._input_obj_verts[i][::5]) # Just doing every 5 points to not collapse gpu mem
                print(touching_indexs)
    ...
    def _forward_G(self, keep_data_for_visuals):
        ...
        # combine losses
        return self._loss_g_CE + self._loss_g_fake + self._loss_g_interpenetration + self._loss_g_fk + self._loss_g_angles + self._loss_g_angles_2ndjoint + self._loss_g_contactloss
