...
class Model(BaseModel):
    ...
    def forward(self, keep_data_for_visuals=False):
        if not self._is_train:
            ...
            for i in range(self._B):
                handfullpose_list, touching_indexs, loss_distance, loss_angle = fk.optimize_hand(handfullpose[i].unsqueeze(0), R[i].unsqueeze(0), T[i].unsqueeze(0), self._input_obj_resampled_verts[i][self._input_object_id[i]])
