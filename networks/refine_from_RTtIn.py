...
class Network(NetworkBase):
    def __init__(self, input_chann=1, output_dim=64):
        ...
        self.fc1 = nn.Linear(64 + 45 + 3*2 + 3 + 12, 128)
        self.fc2 = nn.Linear(128, 128)
        self.fcHR_residual = nn.Linear(128, 45)
        self.fcR = nn.Linear(128, 3)
        self.fcT = nn.Linear(128, 3)

    def forward(self, image_representation, hand_representations, Rs, Ts, n):
        _B = len(hand_representations)
        x_image = self.fc0(image_representation)
        x = self.fc1(torch.cat((x_image, hand_representations, Rs.view(_B, -1), Ts, n), -1))
        x = F.relu(x)
        x = self.fc2(x)
        x = F.relu(x)
