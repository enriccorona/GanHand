...
class Test:
    ...
    def _display_visualizer_test(self, i_epoch, total_steps):
        ...
        results_graspit_post = []
        results_contacts_post = []
        results_avg_contacts_post = []
        results_simulation_post = []
        results_intersection_post = []
        results_plane_intersection = []
        results_taxonomyaccuracy_post = []
        results_taxonomyclassifieraccuracy_post = []
        results_objectaccuracy_post = []
        results_grasppredicting_istouching = []
        results_times = []

        results_predictions_for_percentage_objects = []

        object_taxonomies = np.load('/tmp-network/fast/ecorona/data/YCB_objects/object_taxonomies.npy')

        from sklearn.cluster import MiniBatchKMeans
        taxonomy_poses = np.load('/tmp-network/fast/ecorona/data/YCB_objects/taxonomy_average_classes.npy')
        objects_in_YCB = np.load('/tmp-network/fast/ecorona/data/YCB_objects/objects_in_YCB.npy')

        km_taxonomy = MiniBatchKMeans(n_clusters=21, random_state=1002)
        km_taxonomy.cluster_centers_ = taxonomy_poses[:, 0]
        ...
        print("WE SAMPLE ONE FRAME EVERY %d"%(sampleevery))
        print("WHILE TAKING %d STEPS TO GET THE RIGHT ONE"%(howmanysamples))

        for i_test_batch, test_batch in enumerate(self._dataset_test):
            if i_test_batch % sampleevery != 0:
                continue
            ...
            #hand_faces_gt = np.zeros((0, 3))
            #hand_verts_gt = np.zeros((0, 3))
            #for j in range(numobjects):
            #    try:
            ...
            #        verts = self._model._refined_handpose#[0].cpu().data.numpy()
            #        hand_faces_gt = np.concatenate((hand_faces, self._model._MANO.th_faces.cpu().data.numpy() + len(hand_verts)))
            #        hand_verts_gt = np.concatenate((hand_verts, verts))
            #    except:
            #        pass
            ...
            for j in range(numobjects):
                try:
                    ...
                    for k in range(howmanysamples):
                        ...
                        if len(forces_post) > 3 and finger_is_touching[-1]:
                            sample_hand_verts.append(verts)
                            sample_hand_representations.append(self._model._refined_HR)
                            interpenetration.append(self.get_interpenetration(verts, test_batch['3d_points_object'][0], test_batch['3d_faces_object'][0]))

                            if interpenetration[-1] == 0: # We already have our prediction
                                break

                    if len(interpenetration) == 0:
                        continue

                    ind = np.argmin(interpenetration)
                    if interpenetration[ind] < threshold_intersections:
                        hand_verts.append(sample_hand_verts[ind])
                        hand_representations.append(sample_hand_representations[ind])
                    else:
                        hand_verts.append([])
                        hand_representations.append([])

                except:
                    pass

            inference_time = (time.time() - t)/numobjects

            # GET RESULTS GIVEN PREDICTIONS

            gt_objs = test_batch['3d_classes_object_gt'][0]
            objects_with_predicted_hand = np.zeros(len(gt_objs))



            for j in range(len(hand_verts)):
                verts = hand_verts[j]
                if len(verts) == 0:
                    continue
                object_class = test_batch['id_object'][0][j]
                representation = hand_representations[j]

                if object_class in gt_objs:
                    index = np.where(gt_objs == object_class)[0][0]

                    obj_verts = test_batch['3d_points_object_gt'][0][index]
                    obj_faces = test_batch['3d_faces_object_gt'][0][index]
                    resampled_obj_verts = test_batch['3d_points_resampled_gt'][0][index]

                    forces_post, torques_post, normals_post, finger_is_touching = self.get_contact_points(verts, obj_verts, self._model._MANO.th_faces.cpu().data.numpy(), obj_faces, resampled_obj_verts)

                    if len(forces_post) < 2:
                        results_grasppredicting_istouching.append(0)
                        continue

                    objects_with_predicted_hand[index] = 1
                    ...
                    above_plane, distance = ycb_utils.get_plane_distance(torch.FloatTensor(verts).cuda(), test_batch['plane_eq'][0].float().cuda())
                    if (above_plane==0).sum()>0:
                        plane_interp = -1*(distance*((above_plane==0).float())).mean().cpu().data.numpy()
                    else:
                        plane_interp = 0

                    # Interpenetration
                    interpenetration = 0
                    for k, objv in enumerate(test_batch['3d_points_object_gt'][0]):
                        interpenetration += obman.get_intersection(verts, self._model._MANO.th_faces.cpu().data.numpy(), objv.copy(), test_batch['3d_faces_object_gt'][0][k])
                    ...
                    # Predicted taxonomy
                    taxonomies = object_taxonomies[objects_in_YCB[object_class]]
                    predicted_taxonomy = km_taxonomy.predict(representation.cpu().data.numpy())[0]
                    #predicted_classifier_taxonomy = self._model._class_prediction[0]

                    results_graspit_post.append(grasp_metric)
                    results_contacts_post.append(len(forces_post))
                    results_simulation_post.append(distance)
                    results_intersection_post.append(interpenetration)
                    results_plane_intersection.append(plane_interp)

                    results_taxonomyaccuracy_post.append(taxonomies[predicted_taxonomy])
                    #results_taxonomyclassifieraccuracy_post.append(taxonomies[predicted_classifier_taxonomy])
                    results_objectaccuracy_post.append(1)
                    results_grasppredicting_istouching.append(1)
                    results_times.append(inference_time)
                else:
                    results_objectaccuracy_post.append(1)
            results_predictions_for_percentage_objects.append(np.mean(objects_with_predicted_hand))
        ...
        print("")
        print("NUM SAMPLES")
        print(len(results_graspit_post))

        print("GRASPIT AVG SCORE")
        print(np.mean(results_graspit_post))
        print("AVG NUMBER OF CONTACTS")
        print(np.mean(results_contacts_post))
        print("DISPLACEMENT DISTANCE")
        print(np.mean(results_simulation_post))
        print("INTERSECTION VOLUME")
        print(np.mean(results_intersection_post))
        print("PLANE AVG DIST")
        print(np.mean(results_plane_intersection))

        print("ACCURACY FOR TAXONOMY")
        print(np.mean(results_taxonomyaccuracy_post))
        #print(np.mean(results_taxonomyclassifieraccuracy_post))
        print(np.mean(results_objectaccuracy_post))
        print(np.mean(results_grasppredicting_istouching))

        print("PREDICTION FOR PERCENTAGE OF OBJECTS")
        print(np.mean(results_predictions_for_percentage_objects))
